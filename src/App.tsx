...
import {
  ...
  ClipboardCheck, BarChart3, ChevronDown,
...
const ALL_NAV = [
  { path: '/dashboard',     label: 'Dashboard',    icon: <BarChart3 className="w-3.5 h-3.5" />,      roles: ALL_ROLES },
  { path: '/complaints',    label: 'Complaints',   icon: <ClipboardList className="w-3.5 h-3.5" />,  roles: ALL_ROLES },
  { path: '/inspections',   label: 'Inspections',  icon: <ClipboardCheck className="w-3.5 h-3.5" />, roles: ALL_ROLES },
  { path: '/enforcement',   label: 'Enforcement',  icon: <AlertTriangle className="w-3.5 h-3.5" />,  roles: ['Program Manager', 'Super Admin'] as Role[] },
  { path: '/hearings',      label: "Director's Hearings", icon: <Gavel className="w-3.5 h-3.5" />, roles: ALL_ROLES },
  { path: '/all-locations', label: 'Locations',    icon: <MapPin className="w-3.5 h-3.5" />,         roles: ALL_ROLES },
];
...
function AppShell() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {impersonatedRole && (
        <div className="bg-primary text-primary-foreground px-4 py-1.5 flex items-center justify-between text-xs font-medium">
          <div className="flex items-center gap-2">
            ...
            <span>Previewing as <strong>{impersonatedRole}</strong> — nav and pages reflect this role. Your real data is unchanged.</span>
          </div>
          <button type="button" onClick={handleExitImpersonation}
      ...
      <header className="sticky top-0 z-20 bg-card/95 backdrop-blur-sm border-b border-border shadow-sm h-[57px] flex items-center px-4 sm:px-6 gap-3">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center shadow-md">
            <span className="text-primary-foreground font-black text-[11px] tracking-wide leading-none">EHD</span>
          </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          >
            ...
            <div className="hidden sm:flex flex-col items-end">
              ...
              {realRole && <span className="text-[10px] text-muted-foreground leading-none mt-0.5">{realRole}</span>}
            </div>
          </Link>
          ...
          {isSuperAdmin ? (
            <div className="relative">
              <button type="button" onClick={() => setRolePickerOpen(o => !o)}
              ...
              {rolePickerOpen && (
                <div className="absolute top-full right-0 mt-1.5 bg-card border border-border rounded-xl shadow-lg py-1.5 min-w-[190px] z-30">
                  <p className="px-3 pt-1 pb-1.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Preview as role</p>
                  {ALL_ROLES.map(role => {
                    const isActive = (impersonatedRole ?? realRole) === role;
                    const isReal = role === realRole;
                    return (
                      <button key={role} type="button" onClick={() => handleSetRole(role)}
                        ...
                        <span className="flex items-center gap-1.5">
                          {isReal && <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full">you</span>}
      ...
      {rolePickerOpen && <div className="fixed inset-0 z-20" onClick={() => setRolePickerOpen(false)} />}
      ...
      <main>
        <Routes>
          ...
          <Route path="/inspections/new" element={<InspectionFormPage inspectorName={inspectorName} />} />
          <Route path="/inspections/:complaintId" element={<InspectionFormPage inspectorName={inspectorName} />} />
          ...
          <Route path="/import-complaints" element={<Navigate to="/complaints/import" replace />} />
          ...
          <Route path="/hearing-packets" element={<Navigate to="/enforcement/hearings" replace />} />
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="relative">
            ...
            <div className="relative w-16 h-16 bg-primary rounded-2xl flex items-center justify-center shadow-xl">
              <span className="text-primary-foreground font-black text-base tracking-wide">EHD</span>
            </div>
          </div>
          <div className="text-center space-y-1.5">
            <p className="text-sm font-semibold text-foreground tracking-tight">Environmental Health Division</p>
            <p className="text-xs text-muted-foreground">San Francisco Department of Public Health</p>
          </div>
        </div>
