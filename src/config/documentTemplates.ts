...
export const STATIC_BLOCKS = {
  ...
  common: {
    ...
    contactPrompt: 'You may contact the investigator named below within 24 hours for more information.',
  },
...
export type FormatRule = 'uppercase' | 'date-mm/dd/yyyy' | 'last12-upper' | 'prefix-hhp' | 'bulleted-list' | 'none';
...
export interface DocumentFieldConfig {
  ...
  sourceTable: 'HearingPackets' | 'Complaints' | 'Locations' | 'Violations' | 'Inspections' | 'Users' | 'Chronology' | 'static';
...
export const CHRONOLOGY_VARIABLE_SLOTS: DocumentFieldConfig[] = [
  { key: 'propertyAddress',    label: 'Property Address',       sourceTable: 'Complaints',     sourceField: 'address',               inputType: 'text',     editable: 'auto-filled', required: true  },
  { key: 'blockLot',           label: 'Block/Lot',              sourceTable: 'Locations',      sourceField: 'blockLot',              inputType: 'text',     editable: 'auto-filled', required: false },
  { key: 'facilityName',       label: 'Facility Name (DBA)',    sourceTable: 'Locations',      sourceField: 'dba',                   inputType: 'text',     editable: 'auto-filled', required: false },
  { key: 'dateOfSubmittal',    label: 'Date of Submittal',      sourceTable: 'static',         sourceField: 'today',                 inputType: 'date',     editable: 'locked',      required: false },
  { key: 'ownerName',          label: 'Property Owner',         sourceTable: 'Locations',      sourceField: 'ownerName',             inputType: 'text',     editable: 'auto-filled', required: false },
  { key: 'ownerPhone',         label: 'Owner Phone',            sourceTable: 'Locations',      sourceField: 'ownerPhone',            inputType: 'text',     editable: 'auto-filled', required: false },
  { key: 'ownerEmail',         label: 'Owner Email',            sourceTable: 'Locations',      sourceField: 'ownerEmail',            inputType: 'email',    editable: 'auto-filled', required: false },
  { key: 'responsibleParty',   label: 'Responsible Party',      sourceTable: 'Locations',      sourceField: 'responsibleParty',      inputType: 'text',     editable: 'editable',    required: false },
  { key: 'responsiblePhone',   label: 'Responsible Party Phone',sourceTable: 'Locations',      sourceField: 'responsiblePartyPhone', inputType: 'text',     editable: 'auto-filled', required: false },
  { key: 'responsibleEmail',   label: 'Responsible Party Email',sourceTable: 'Locations',      sourceField: 'responsiblePartyEmail', inputType: 'email',    editable: 'auto-filled', required: false },
  { key: 'hearingDate',        label: 'Hearing Date',           sourceTable: 'HearingPackets', sourceField: 'hearingDate',           inputType: 'date',     editable: 'auto-filled', required: false },
  { key: 'programCode',        label: 'Program Code',           sourceTable: 'HearingPackets', sourceField: 'programCode',           inputType: 'dropdown', editable: 'editable',    required: false, options: PROGRAM_CODES },
  { key: 'caseNumber',         label: 'Case Number',            sourceTable: 'HearingPackets', sourceField: 'caseNumber',            inputType: 'text',     editable: 'editable',    required: false },
  { key: 'entries',            label: 'Chronology Entries',     sourceTable: 'Chronology',     sourceField: '*',                     inputType: 'repeater', editable: 'auto-filled', required: false },
  { key: 'entries[].date',     label: 'Entry Date',             sourceTable: 'Chronology',     sourceField: 'entryDate',             inputType: 'date',     editable: 'auto-filled', required: false },
  { key: 'entries[].code',     label: 'Code Section',           sourceTable: 'Chronology',     sourceField: 'citationCode',          inputType: 'text',     editable: 'editable',    required: false },
  { key: 'entries[].summary',  label: 'Summary',                sourceTable: 'Chronology',     sourceField: 'summary',               inputType: 'longText', editable: 'auto-filled', required: false },
  { key: 'entries[].exhibits', label: 'Exhibit Refs',           sourceTable: 'Chronology',     sourceField: 'exhibitRefs',           inputType: 'text',     editable: 'editable',    required: false },
  { key: 'pageCount',          label: 'Supplemental Page Count',sourceTable: 'static',         sourceField: 'computed',              inputType: 'text',     editable: 'locked',      required: false, hint: 'Auto-calculated from chronology entry pagination' },
  { key: 'recommendedOrder',   label: 'Recommended Order Text', sourceTable: 'HearingPackets', sourceField: 'hearingOrderData',      inputType: 'longText', editable: 'editable',    required: false },
  { key: 'inspectorName',      label: 'Inspector Name',         sourceTable: 'Users',          sourceField: 'firstName+lastName',    inputType: 'text',     editable: 'auto-filled', required: false },
  { key: 'inspectorSignature', label: 'Inspector Signature',    sourceTable: 'HearingPackets', sourceField: 'inspectorSignature',    inputType: 'text',     editable: 'locked',      required: false, hint: 'Auto-filled from the inspector\'s saved signature' },
  { key: 'inspectorEmail',     label: 'Inspector Email',        sourceTable: 'Users',          sourceField: 'email',                 inputType: 'email',    editable: 'auto-filled', required: false },
  { key: 'inspectorPhone',     label: 'Inspector Phone',        sourceTable: 'static',         sourceField: 'common.investigatorDefaultPhone', inputType: 'text', editable: 'locked', required: false },
  { key: 'managerName',        label: 'Manager / Supervisor',   sourceTable: 'Users',          sourceField: 'firstName+lastName',    inputType: 'text',     editable: 'auto-filled', required: false },
  { key: 'managerSignature',   label: 'Manager Signature',      sourceTable: 'HearingPackets', sourceField: 'managerSignature',      inputType: 'text',     editable: 'locked',      required: false, hint: 'Applied by a Program Manager or Admin via the Countersign button' },
];
