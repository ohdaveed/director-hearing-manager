  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden flex flex-col">
      ...
      <div className="relative aspect-[4/3] bg-muted">
        ...
        {photo.savedToDb && (
          <span className="absolute bottom-2 left-2 text-[10px] bg-success/90 text-background px-1.5 py-0.5 rounded font-medium">
            Saved
          </span>
        )}
        {photo.error && (
          <span className="absolute bottom-2 left-2 text-[10px] bg-destructive/90 text-destructive-foreground px-1.5 py-0.5 rounded font-medium">
            {photo.error}
          </span>
        )}
      </div>
      ...
      <div className="p-3 space-y-2 flex-1">
        {/* Type Badge */}
        <div
          className={`inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-full border ${PHOTO_TYPE_THEME[photo.photoType]}`}
        >
          {photo.photoType}
        </div>
        ...
        {showViolationSelect && (
          <Select
            value={photo.violationLabel}
            onValueChange={(v) => onChange(photo.id, "violationLabel", v)}
          >
            <SelectTrigger className="h-8 text-xs">
