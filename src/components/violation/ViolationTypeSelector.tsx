...
export default function ViolationTypeSelector({ value, onChange, readOnly }: Props) {
  ...
  const grouped = VIOLATION_CATEGORIES.map((cat) => ({
    cat,
    items: filtered.filter((v) => v.category === cat),
                ...
                {items.map((v) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`text-xs font-mono shrink-0 ${isSel ? "text-primary/70" : "text-muted-foreground"}`}
                      >
                        {v.code}
                      </span>
                    </button>
