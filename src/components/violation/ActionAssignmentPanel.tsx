...
import {
  Building2, User, ChevronDown, ChevronUp, Plus, X,
...
export default function ActionAssignmentPanel({
  ...
  const { selectedItems, unselectedItems, showDivider } = useActionSorting(predefinedActions, selectedTexts);
  const selectedCount = selectedTexts.size;
  const hasCustom = customText.trim().length > 0;
  const hasContent = selectedCount > 0 || hasCustom;
  const allSelected = predefinedActions.length > 0 && predefinedActions.every(a => selectedTexts.has(a.text));
  ...
  }, [expandTrigger]); // eslint-disable-line

  if (readOnly && !hasContent) return null;
  if (!readOnly && predefinedActions.length === 0 && !hasContent) return null;

  const handleSelectAll = () => predefinedActions.forEach(a => { if (!selectedTexts.has(a.text)) onToggle(a.text); });
  const handleClearAll = () => predefinedActions.forEach(a => { if (selectedTexts.has(a.text)) onToggle(a.text); });
    ...
    return (
      ...
      >
        <span className={`mt-0.5 shrink-0 transition-colors ${
          checked ? 'text-primary' : 'text-border group-hover:text-muted-foreground'
        }`}>
  ...
  return (
    <div ref={panelRef} className={`rounded-xl border overflow-hidden transition-all ${
      ...
      >
        ...
        {selectedCount > 0 && (
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${ACTION_PARTY_THEME[party].badge}`}>
            {selectedCount} ✓
          </span>
        )}
        {!readOnly && (
        ...
        <div className="bg-card">
          ...
          {!readOnly && predefinedActions.length > 1 && (
            <div className="flex items-center gap-2 px-3.5 py-2 border-b border-border/60 bg-muted/20">
              ...
              <button type="button" onClick={handleClearAll} disabled={selectedCount === 0}
              ...
              <span className="ml-auto text-[10px] text-muted-foreground tabular-nums">
                {predefinedActions.length} action{predefinedActions.length !== 1 ? 's' : ''}
              </span>
            </div>
          )}
          ...
          <div className="px-3.5 pb-3.5">
            {(showCustom || hasCustom) ? (
              <div className="mt-3 space-y-1.5">
                ...
                {readOnly ? (
                  <Textarea value={customText} className="min-h-[60px] resize-none text-xs" disabled />
                ) : (
                  <>
                  <div className={`border rounded-lg overflow-hidden ${customError ? 'border-destructive' : 'border-border'}`}>
                    <Textarea
                      value={customText}
                    ...
                    <div className="border-t border-border bg-muted/30 px-3 py-1.5 flex items-center gap-1.5 flex-wrap">
                      <span className="text-[10px] text-muted-foreground font-semibold shrink-0">Snippets:</span>
                      ...
                      {!showAllSnippets && (
                        <button type="button" onClick={() => setShowAllSnippets(true)}
