...
export default function ExhibitCard({
  letter, fileName, category, pageCount, pageRange,
  fileUrl, entries, onDelete, onLinkToEntry, isDeleting,
}: Props) {
  return (
    <div className="border border-border rounded-lg p-3 bg-card shadow-sm">
      ...
      <p className="text-[10px] text-foreground truncate leading-snug" title={fileName}>{fileName}</p>

      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
        {category && (
          <span className="text-[10px] bg-muted text-muted-foreground px-1.5 py-0.5 rounded">{category}</span>
        )}
            ...
            <SelectContent>
              {entries.map((e, i) => (
                <SelectItem key={e.id} value={e.id} className="text-[10px]">
                  {String.fromCharCode(65 + i)} — {e.entryType || 'Entry'}{e.entryDate ? ` (${e.entryDate})` : ''}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
