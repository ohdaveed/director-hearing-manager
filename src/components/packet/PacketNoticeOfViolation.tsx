...
export function PacketNoticeOfViolation({ packet, complaint, location, inspections, inspector }: Props) {
  // ── Resolve auto-filled variables from database records ───────────────────

  // Variable slot: Locations.ownerName (auto-filled, uppercase)
  const ownerName = location?.ownerName ?? '';
  // Variable slot: Locations.ownerAddress (auto-filled, uppercase)
  const ownerAddress = location?.ownerAddress ?? '';
  // Variable slot: Locations.responsibleParty (editable)
  const responsibleParty = location?.responsibleParty ?? '';
  // Variable slot: Locations.blockLot parsed → block, lot (auto-filled)
  const { block, lot } = parseBlockLot(location?.blockLot);
  ...
  // Variable slot: Inspections.inspectionDate — earliest date as NOV date (auto-filled)
  const novDate = inspections.length > 0
    ? fmtDate(inspections[0].inspectionDate)
    ...
    for (const v of insp.violations) {
      ...
      if (key && !violationMap.has(key)) {
        violationMap.set(key, {
          ...
          label: v.violationLabel ?? '',        // Violations.violationLabel (auto-filled)
          correctiveAction: v.correctiveAction ?? '', // Violations.correctiveAction (editable)
          dueDate: v.dueDate,                   // Violations.dueDate (editable)
        });
      }
    }
  }
    ...
    <>
      {rows.length === 0 ? (
        ...
        <tr key={i}>
          ...
          {/* Column 2: Steps to Abate — editable corrective action from Violations.correctiveAction */}
          <td style={CELL_STYLE}>
            {v.correctiveAction
          ...
          {/* Column 3: Deadline — editable Violations.dueDate */}
  ...
  return (
    <>
      ...
      <div className="packet-page print-page-break" style={baseStyle}>
        ...
        {/* Responsible party — variable slot: Locations.responsibleParty (editable) */}
        {responsibleParty && (
        ...
        <div style={{ fontSize: pt.contactBlock, marginTop: 'auto' }}>
          ...
          {/* Variable: Users.firstName + lastName (matched via Complaints.assignedTo) */}
          <p style={{ margin: '0 0 1px' }}>
            <strong>Investigator:</strong> {inspector?.name ?? complaint?.assignedTo ?? ''}
          </p>
