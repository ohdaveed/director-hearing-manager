...
function getProgramChecks(assignedProgram: string | undefined, categories: string[] | undefined) {
  ...
  return {
    ...
    other:
      ...
      !prog.includes("food") &&
      !prog.includes("massage") &&
      !prog.includes("tobacco"),
  };
}
...
export function PacketNoticeOfHearing({
  packet,
  complaint,
  location,
  inspector,
  ...
  const permitNum = getPermitNumber(complaint, location);
  ...
  const hearingDateTime = [fmtDate(packet.hearing_date), packet.hearingTime]
    .filter(Boolean)
    .join(", ");

  return (
    <div
      className="packet-page print-page-break relative"
      style={{
        fontFamily: "Times New Roman, serif",
        fontSize: "11pt",
        lineHeight: 1.5,
    ...
    >
      ...
      >
        <span>
          <PrintCheckbox checked={prog.food} /> Food
        </span>
        <span>
          <PrintCheckbox checked={prog.housing} /> Housing
        </span>
        <span>
          <PrintCheckbox checked={prog.massage} /> Massage
        </span>
        <span>
          <PrintCheckbox checked={prog.tobacco} /> Tobacco
        </span>
        <span>
          <PrintCheckbox checked={prog.solidWaste} /> Solid Waste
        </span>
        <span>
