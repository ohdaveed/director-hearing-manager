...
export type HearingOrderData = {
  ...
  determinations: { codeSection: string; determination: 'upheld' | 'dismissed' | 'modified'; notes: string }[];
  permitNumber: string;
...
const BOX: React.CSSProperties = { border: '1px solid black', padding: '8px 12px', marginBottom: '12px' };
const LABEL: React.CSSProperties = { fontSize: '9pt', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '4px' };
const UNDERLINE: React.CSSProperties = { borderBottom: '1px solid black', minHeight: '22px', marginBottom: '4px', paddingBottom: '2px' };
  ...
  return (
    <div className="packet-page print-page-break" style={{ fontFamily: 'Times New Roman, serif', fontSize: '11pt', lineHeight: 1.5 }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '20px', borderBottom: '2px solid black', paddingBottom: '12px' }}>
        <p style={{ fontSize: '10pt', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          City and County of San Francisco — Department of Public Health
        </p>
        <h2 style={{ fontSize: '15pt', fontWeight: 'black', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '10px 0 4px' }}>
      ...
      <div style={BOX}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 24px' }}>
          <div>
            <p style={{ fontSize: '9pt', margin: '0 0 2px', fontWeight: 'bold' }}>Address of Violation</p>
          ...
          {rpName !== location?.ownerName && (
            <>
              ...
              <div>
                <p style={{ fontSize: '9pt', margin: '0 0 2px', fontWeight: 'bold' }}>RP Address</p>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10pt' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid black' }}>
                <th style={{ textAlign: 'left', padding: '2px 8px', fontSize: '9pt', textTransform: 'uppercase' }}>Name</th>
                <th style={{ textAlign: 'left', padding: '2px 8px', fontSize: '9pt', textTransform: 'uppercase' }}>Role</th>
                <th style={{ textAlign: 'center', padding: '2px 8px', fontSize: '9pt', textTransform: 'uppercase' }}>Attended</th>
              </tr>
            </thead>
            <tbody>
      ...
      <div style={BOX}>
        ...
        {orderData.determinations.length > 0 ? (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10pt' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid black' }}>
                <th style={{ textAlign: 'left', padding: '2px 8px', fontSize: '9pt', textTransform: 'uppercase', width: '140px' }}>Code Section</th>
                <th style={{ textAlign: 'left', padding: '2px 8px', fontSize: '9pt', textTransform: 'uppercase', width: '120px' }}>Determination</th>
                <th style={{ textAlign: 'left', padding: '2px 8px', fontSize: '9pt', textTransform: 'uppercase' }}>Notes / Conditions</th>
              </tr>
            </thead>
            <tbody>
              {orderData.determinations.map((d, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #ddd', verticalAlign: 'top' }}>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace', fontSize: '10pt' }}>{d.codeSection || codeSections[i] || '—'}</td>
                  <td style={{ padding: '5px 8px', fontWeight: 'bold', fontSize: '10pt', textTransform: 'capitalize' }}>{d.determination}</td>
        ...
        <div style={BOX}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 24px' }}>
            {orderData.permitNumber && (
              <div>
                <p style={{ fontSize: '9pt', margin: '0 0 2px', fontWeight: 'bold' }}>Permit Number</p>
                <div style={UNDERLINE}>{orderData.permitNumber}</div>
              </div>
            )}
            {orderData.permitDecision && (
              <div>
                <p style={{ fontSize: '9pt', margin: '0 0 2px', fontWeight: 'bold' }}>Permit Decision</p>
                <div style={UNDERLINE}>{orderData.permitDecision}</div>
              </div>
            )}
            {orderData.reinspectionFee && (
              <div>
                <p style={{ fontSize: '9pt', margin: '0 0 2px', fontWeight: 'bold' }}>Reinspection Fee</p>
      ...
      {orderData.nuisanceAbatementConditions && (
        <div style={BOX}>
          ...
          <p style={{ fontSize: '10.5pt', whiteSpace: 'pre-wrap' }}>{orderData.nuisanceAbatementConditions}</p>
        </div>
      )}
      ...
      {/* Signature block */}
      <div style={{ marginTop: '28px', display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
        <div>
          <div style={{ borderBottom: '1px solid black', minHeight: '32px', marginBottom: '4px' }}>
