...
export default function PhotoPrintSection({ photos, address, inspectionDate, complaintId }: Props) {
  ...
  const headerText = [address, inspectionDate, complaintId ? `Complaint #${complaintId}` : ''].filter(Boolean).join('  ·  ');

  return (
    <div className="print-photo-section">
      {pages.map((page, pi) => (
        <div key={pi} className="print-page" style={{ pageBreakBefore: pi === 0 ? 'always' : 'always', padding: '24px' }}>
          {/* Page Header */}
          <div style={{ borderBottom: '2px solid #1d4ed8', paddingBottom: '8px', marginBottom: '20px' }}>
            ...
            <div style={{ fontSize: '10px', color: '#6b7280', marginTop: '1px' }}>
              Page {pi + 1} of {pages.length} (Photos {pi * 3 + 1}–{Math.min((pi + 1) * 3, sorted.length)} of {sorted.length})
            </div>
          </div>
