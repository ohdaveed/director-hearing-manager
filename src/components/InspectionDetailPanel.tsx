...
export default function InspectionDetailPanel({ inspectionId, onClose }: Props) {
  ...
  const [notes, setNotes] = useState<{ observations?: { id: string; text: string; linkedViolationKey: string }[] } | null>(null);
    ...
    getInspectionDetail({ inspectionId })
      .then(d => {
        ...
        if (d.inspection.notes) {
          try { setNotes(JSON.parse(d.inspection.notes)); } catch { }
        }
      })
      .catch(() => {})
  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-muted/30">
        <div>
          <p className="font-semibold text-foreground text-base">{inspection.facilityAddress || 'Inspection Detail'}</p>
          <div className="flex items-center gap-2 mt-0.5">
            {isSubmitted ? (
              <span className="inline-flex items-center gap-1 text-xs font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-full"><CheckCircle2 className="w-3 h-3" /> Submitted</span>
            ) : (
          ...
          <div>
            ...
            <div className="space-y-2">
              {notes.observations.map((obs, i) => (
                <div key={obs.id} className="border border-border rounded-lg px-3 py-2.5 text-sm text-foreground">
