...
export default function PhotoUploadSection({ complaintId, complaintRecordId, inspectionRecordId, inspector, photos, onPhotosChange }: Props) {
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      ...
      try {
        ...
        onPhotosChange(prev => prev.map(p =>
          p.id === entry.id ? { ...p, uploading: false, uploadedUrl: fileUrl, savedToDb: !!complaintId } : p
        ));
      } catch {
        onPhotosChange(prev => prev.map(p =>
      ...
      <div className="p-6 space-y-5">
        {!complaintId && (
          <div className="flex items-start gap-2 bg-warning/10 border border-warning/30 rounded-lg px-4 py-3 text-sm text-warning">
            ...
            <span>Enter a <strong>Complaint ID</strong> above to save photos permanently to the database.</span>
          </div>
        )}
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">JPG, PNG, HEIC, WebP — files over 25MB auto-resized at 600 DPI quality</p>
          </div>
        ...
        {photos.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {photos.map(photo => (
              <PhotoCard key={photo.id} photo={photo} onChange={handleChange} onRemove={handleRemove} />
            ))}
          </div>
        )}
      </div>
