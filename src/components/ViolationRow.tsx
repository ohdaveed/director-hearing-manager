...
export default function ViolationRow({
  violation, index, inspectionDate, readOnly, onChange, onRemove,
  expandOwnerTrigger, expandTenantTrigger,
}: Props) {
  ...
  const selectedType = VIOLATION_TYPES.find(v => `${v.category}||${v.label}` === violation.violationKey);
  ...
  const tenantPredefined = selectedType?.correctiveActions?.filter(a => a.party === 'Tenant') ?? [];
  ...
  useEffect(() => {
    if (!initDoneRef.current) {
      ...
      if (ownerA.length || tenantA.length) {
        ...
        const predTenant = new Set(predefined.filter(a => a.party === 'Tenant').map(a => a.text));
        const ownerSel = new Set<string>(); const ownerCustomItems: string[] = [];
        ownerA.forEach(t => predOwner.has(t) ? ownerSel.add(t) : ownerCustomItems.push(t));
        const tenantSel = new Set<string>(); const tenantCustomItems: string[] = [];
      ...
      const restoredCustom = (violation.selectedObservations ?? []).filter(o => !predefinedObsTexts.has(o));
      if (restoredCustom.length > 0) {
        setCustomObsList(restoredCustom);
        const initMap: CustomObsActions = {};
        restoredCustom.forEach(o => { initMap[o] = { owner: new Set(), tenant: new Set() }; });
  ...
  const buildOwnerActions = (sel: Set<string>, custom: string, obsActions: CustomObsActions = customObsActions) => {
    const ordered = ownerPredefined.filter(a => sel.has(a.text)).map(a => a.text);
    Object.values(obsActions).forEach(({ owner }) => {
      owner.forEach(a => { if (!ordered.includes(a)) ordered.push(a); });
    });
    if (custom.trim()) ordered.push(custom.trim());
    return ordered;
  };

  const buildTenantActions = (sel: Set<string>, custom: string, obsActions: CustomObsActions = customObsActions) => {
    const ordered = tenantPredefined.filter(a => sel.has(a.text)).map(a => a.text);
    Object.values(obsActions).forEach(({ tenant }) => {
      tenant.forEach(a => { if (!ordered.includes(a)) ordered.push(a); });
    });
    if (custom.trim()) ordered.push(custom.trim());
    return ordered;
  };
  ...
  const handleToggleObservation = (obsText: string) => {
    ...
    if (obs && isAdding) {
      if (obs.autoOwnerActions?.length) {
        ...
        setAutoOwner(prev => { const n = new Set(prev); obs.autoOwnerActions!.forEach(a => n.add(a)); return n; });
      ...
      if (obs.autoTenantActions?.length) {
        ...
        setAutoTenant(prev => { const n = new Set(prev); obs.autoTenantActions!.forEach(a => n.add(a)); return n; });
  ...
  const handleAddCustomObs = () => {
    ...
    const newActions: CustomObsActions = { ...customObsActions, [text]: { owner: new Set(), tenant: new Set() } };
  ...
  const handleRemoveCustomObs = (text: string) => {
    ...
    setSelectedObs(nextObs);
    onChange(violation.id, 'selectedObservations', nextObs);
    onChange(violation.id, 'ownerActions', buildOwnerActions(selectedOwnerActions, ownerCustom, newActions));
    onChange(violation.id, 'tenantActions', buildTenantActions(selectedTenantActions, tenantCustom, newActions));
  };

  const handleToggleCustomObsAction = (obsText: string, actionText: string, party: 'Owner' | 'Tenant') => {
    const entry = customObsActions[obsText] ?? { owner: new Set<string>(), tenant: new Set<string>() };
    const newEntry = {
      owner: new Set(entry.owner),
      tenant: new Set(entry.tenant),
    };
    if (party === 'Owner') {
      newEntry.owner.has(actionText) ? newEntry.owner.delete(actionText) : newEntry.owner.add(actionText);
    } else {
      newEntry.tenant.has(actionText) ? newEntry.tenant.delete(actionText) : newEntry.tenant.add(actionText);
    }
    const newActions: CustomObsActions = { ...customObsActions, [obsText]: newEntry };
    setCustomObsActions(newActions);
    onChange(violation.id, 'ownerActions', buildOwnerActions(selectedOwnerActions, ownerCustom, newActions));
    onChange(violation.id, 'tenantActions', buildTenantActions(selectedTenantActions, tenantCustom, newActions));
  };
  ...
  const statusStyle = (s: typeof STATUSES[number]) => {
    if (s !== currentStatus) return 'bg-background text-muted-foreground hover:bg-muted';
    ...
    <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 pt-4 pb-3">
        ...
        <div className="flex-1 min-w-0">
          <ViolationTypeSelector value={violation.violationKey} onChange={handleViolationSelect} readOnly={readOnly} />
        </div>
        {violation.isAuto && (
        ...
        {!readOnly && (
          <button type="button" onClick={() => onRemove(violation.id)}
      ...
      {selectedType && (
        <div className="px-4 pb-3 flex items-center gap-2 flex-wrap border-b border-border/60">
          ...
          {badge && (
            <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded border ${badge.cls}`}>
        ...
        <div className="px-4 py-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              {readOnly ? (
                ...
                <>
                  ...
                  {((selectedType.observations?.length ?? 0) > 0 || customObsList.length > 0) && (
                    <div className="flex flex-wrap gap-2">
                      {/* Predefined observation chips */}
                      {(selectedType.observations ?? []).map(obs => {
                        const active = selectedObs.includes(obs.text);
                        const count = (obs.autoOwnerActions?.length ?? 0) + (obs.autoTenantActions?.length ?? 0);
                        return (
                          <button
                            key={obs.text}
                            type="button"
                  ...
                  {customObsList.map(obsText => {
                    ...
                    return (
                      <div key={`picker-${obsText}`} className="pl-3 border-l-2 border-border/50 space-y-1.5">
                          ...
                          {ownerPredefined.map(a => (
                            ...
                            >
                              <Checkbox
                                ...
                                onCheckedChange={() => handleToggleCustomObsAction(obsText, a.text, 'Owner')}
                                className="w-3 h-3"
                              />
                              <span className="text-muted-foreground font-semibold text-[10px]">Owner</span>
                              <span>{a.text}</span>
                            </label>
                          ))}
                          {tenantPredefined.map(a => (
                            ...
                            >
                              <Checkbox
                                ...
                                onCheckedChange={() => handleToggleCustomObsAction(obsText, a.text, 'Tenant')}
                                className="w-3 h-3"
                              />
                              <span className="text-muted-foreground font-semibold text-[10px]">Tenant</span>
                              <span>{a.text}</span>
                            </label>
                          ))}
                        </div>
                  ...
                  <div className="flex items-center gap-2 pt-0.5">
                    <Input
                      ...
                      onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); handleAddCustomObs(); } }}
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div className="space-y-1">
              ...
              <Input placeholder="e.g. Unit 3B, Basement" value={violation.location}
