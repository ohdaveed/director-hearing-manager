...
interface Props {
  ...
  contactDates: string; setContactDates: (v: string) => void;
...
export default function ComplainantSection({
  hasComplainantInfo, anonymous, setAnonymous,
  name, setName, phone, setPhone, email, setEmail,
  address, setAddress, contactDates, setContactDates,
  touched, formErrors, blurField, clearError,
}: Props) {
  return (
    <div className="bg-card border border-border rounded-xl shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
      <div className="flex items-center gap-2 mb-5">
        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${hasComplainantInfo ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
      ...
      <label className="flex items-center gap-2.5 cursor-pointer select-none mb-4 p-3 rounded-lg bg-muted/50 border border-border">
        ...
        {anonymous && <span className="ml-auto text-xs bg-muted px-2 py-0.5 rounded-full text-muted-foreground font-medium">Anonymous ☑</span>}
      </label>
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <Input placeholder="(415) 555-5678" value={phone} onChange={e => setPhone(e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1">
                ...
                <Input
                  ...
                  value={email}
                  onChange={e => { setEmail(e.target.value); if (validateEmail(e.target.value)) clearError('complainantEmail'); }}
                  onBlur={() => blurField('complainantEmail')}
                  className={touched.complainantEmail && formErrors.complainantEmail ? 'border-destructive' : ''}
                />
                <AnimatePresence>
                  {touched.complainantEmail && formErrors.complainantEmail && <FieldError message={formErrors.complainantEmail} />}
                </AnimatePresence>
              </div>
              <div className="sm:col-span-2 space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1">
              ...
              <div className="sm:col-span-2 space-y-1">
                ...
                <Input placeholder="e.g. 4/2/26, 4/15/26" value={contactDates} onChange={e => setContactDates(e.target.value)} />
              </div>
            </div>
          </motion.div>
        )}
