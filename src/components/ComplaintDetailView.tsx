...
import {
  Loader2, FileEdit, FilePlus, CheckCircle2, XCircle, Clock,
  Camera, ClipboardList, User, Phone, Mail, Calendar, AlertCircle, Lock,
  Home, Save, ExternalLink, Pencil, Search, Link2,
} from 'lucide-react';
import ComplaintChronologyPanel from './ComplaintChronologyPanel';
import {
  ACTIVE_STATUSES, CLOSURE_STATUSES,
  STATUS_DESCRIPTIONS,
...
function DescriptionText({ text }: { text: string }) {
  ...
  const display = isLong && !expanded ? text.slice(0, DESC_THRESHOLD) + '…' : text;
  return (
    <div className="mt-1">
...
function SectionHeader({ icon, title, count, right }: {
  icon: React.ReactNode; title: string; count?: number; right?: React.ReactNode;
}) {
  return (
...
export default function ComplaintDetailView({
  ...
  const statusSelector = (
    <Select value={currentStatus || '_'} onValueChange={handleStatusChange} disabled={updatingStatus}>
      ...
      <SelectContent>
        <div className="px-2 pt-2 pb-1 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Active</div>
        {ACTIVE_STATUSES.map(s => (
          <SelectItem key={s} value={s}>
            <span className="font-medium">{s}</span>
            <span className="hidden sm:inline text-xs text-muted-foreground ml-2">{STATUS_DESCRIPTIONS[s]}</span>
          </SelectItem>
        ))}
        <div className="px-2 pt-3 pb-1 text-[10px] font-bold text-muted-foreground uppercase tracking-widest border-t border-border mt-1">Closure</div>
          ...
          <div className="space-y-3">
            ...
            {canEditStatus && (
              <div className="space-y-2">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Search and link a location</p>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground" />
                  <Input
                    value={locationSearch}
                    onChange={e => { setLocationSearch(e.target.value); debouncedLocationSearch(e.target.value); }}
                ...
                {locationSearch.length >= 2 && !locationSearching && locationResults.length === 0 && (
                  <p className="text-xs text-muted-foreground italic">No locations found for &ldquo;{locationSearch}&rdquo;</p>
                )}
              </div>
            )}
          </div>
        ...
        {detail?.location && !rpEditing && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {rpName || rpAddress || rpPhone || rpEmail ? (
              <>
                <InfoRow icon={<User className="w-3.5 h-3.5" />} label="Owner / Responsible Party" value={rpName} />
                <InfoRow icon={<Home className="w-3.5 h-3.5" />} label="Mailing Address" value={rpAddress} />
              ...
              <p className="text-xs text-muted-foreground italic col-span-2">
                ...
                {canEditStatus && (
                  <button onClick={() => setRpEditing(true)} className="text-primary hover:underline">Add from SF PIM</button>
                )}
              </p>
            )}
          </div>
        ...
        {detail?.location && rpEditing && (
          <div className="space-y-3">
            <p className="text-xs text-muted-foreground mb-3">
              ...
              <a href="https://sfplanninggis.org/pim/" target="_blank" rel="noopener noreferrer"
                className="text-primary hover:underline">sfplanninggis.org/pim</a>.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Owner / Responsible Party</label>
                <Input value={rpName} onChange={e => setRpName(e.target.value)} placeholder="e.g. Jane Doe" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Mailing Address</label>
                <Input value={rpAddress} onChange={e => setRpAddress(e.target.value)} placeholder="e.g. 123 Main Street, SF CA" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Phone</label>
                <Input value={rpPhone} onChange={e => setRpPhone(e.target.value)} placeholder="e.g. (415) 555-1234" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Email</label>
                <Input value={rpEmail} onChange={e => setRpEmail(e.target.value)} placeholder="e.g. owner@email.com" type="email" />
              </div>
            </div>
  ...
  const inspectionHistory = loading ? (
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      <div className="px-5 py-3 bg-muted/40 border-b border-border"><Skeleton className="h-4 w-40" /></div>
    ...
    <div className="bg-card border border-border rounded-xl p-8 text-center text-muted-foreground shadow-sm">
      ...
      {canStartInspection && <p className="text-xs mt-1">Click &ldquo;Start Inspection&rdquo; to begin.</p>}
    </div>
  ) : null;
            ...
            {blockedViolations && blockedViolations.length > 0 && (
              <div className="bg-destructive/5 border border-destructive/20 rounded-lg p-3">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-destructive">Cannot mark as Closed — Compliant</p>
                    <ul className="mt-2 space-y-1">
                      {blockedViolations.map((v, i) => (
                        <li key={i} className="flex items-start gap-1.5 text-xs text-foreground">
                          <XCircle className="w-3 h-3 text-destructive flex-shrink-0 mt-0.5" />{v}
                        </li>
                      ))}
                    </ul>
                  </div>
  ...
  return (
    <div className="w-full">
        ...
        <div className="space-y-4 lg:col-span-7">
          ...
          {loading ? (
            <div className="bg-card border border-border rounded-xl p-5 space-y-3">
              <Skeleton className="h-4 w-1/3" /><Skeleton className="h-4 w-1/2" /><Skeleton className="h-4 w-2/5" />
            </div>
          ) : detail && (
            <div className="bg-card border border-border rounded-xl p-5 shadow-sm">
      ...
      {/* Photos — full width below both columns */}
      {!loading && detail && detail.photos.length > 0 && (
        <div className="mt-4 bg-card border border-border rounded-xl overflow-hidden shadow-sm">
          <SectionHeader
          ...
          <div className="container mx-auto px-4 py-3 max-w-[1300px]">
            <div className="flex items-center gap-3">
              ...
              {canStartInspection && (
                <Button onClick={handleStartInspection} size="sm" className="shrink-0 gap-1.5">
                  {hasDraft ? <FileEdit className="w-3.5 h-3.5" /> : <FilePlus className="w-3.5 h-3.5" />}
            ...
            {blockedViolations && blockedViolations.length > 0 && (
              <p className="mt-1.5 text-xs text-destructive flex items-center gap-1.5">
                ...
                Cannot close — {blockedViolations.length} unresolved violation{blockedViolations.length !== 1 ? 's' : ''}
              </p>
            )}
          </div>
        </div>
