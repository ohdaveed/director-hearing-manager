...
function makeDefaultOrderData(
  ...
  return {
    ...
    determinations: codeSections.map((cs: any) => ({
      codeSection: String(cs ?? ""),
      determination: "upheld" as const,
      notes: "",
    })),
    permitNumber: "",
    permitDecision: "",
...
export default function HearingOrderEditor({
  ...
  const updateAttendee = (
    i: number,
    field: keyof HearingOrderData["attendees"][0],
    val: string | boolean,
  ) => {
    setOrderData((prev) => ({
      ...prev,
  ...
  const addDetermination = () =>
    setOrderData((prev) => ({
      ...
      determinations: [
        ...prev.determinations,
        { codeSection: "", determination: "upheld", notes: "" },
      ],
    }));
  ...
  const removeDetermination = (i: number) =>
    setOrderData((prev) => ({
      ...prev,
  ...
  const handleSave = async () => {
    ...
    try {
      await packetService.update(packet.id, {
        hearing_order_data: JSON.stringify(orderData),
        hearing_order_date: orderData.orderDate || undefined,
      });
        ...
        <div className="print-toolbar sticky top-0 z-10 bg-card border-b border-border shadow-md px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-foreground">
              Director's Hearing Order — Preview
            </h2>
          </div>
