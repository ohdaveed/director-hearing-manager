...
import { Loader2, Save, CheckCircle2, ChevronLeft, Plus, Camera, Printer, Wand2, XCircle, AlertTriangle, X, Clock } from 'lucide-react';
import {
...
function makeDefaultState(complaintId: string): FormState {
  ...
  return {
    complaintid: complaintId, inspection_date: date, timeIn, timeOut,
    inspection_type: '', rating: 'Satisfactory', areasInspected: [], violations: [],
...
export default function InspectionFormPage({ inspectorName }: Props) {
  ...
  const sortedChipKeys = useMemo(() => {
    const matchedLabels = new Set((selectedComplaint?.category ?? []).map((c: string) => c.toLowerCase()));
  ...
  const handleSelectComplaint = useCallback((complaint: any) => {
  ...
  const fillInspectionDemoData = () => {
    if (!form) return;
    const demo = generateInspectionDemoData(form.inspection_date);
      ...
      <div className="container mx-auto px-3 sm:px-6 py-6 max-w-5xl">
        <div className="flex items-start justify-between mb-6">
          <div className="space-y-2">
            <Skeleton className="h-4 w-32" /><Skeleton className="h-7 w-64" /><Skeleton className="h-4 w-48" />
          </div>
          <Skeleton className="h-9 w-24" />
        </div>
        <div className="space-y-4">
          <Skeleton className="h-40 w-full rounded-xl" /><Skeleton className="h-40 w-full rounded-xl" /><Skeleton className="h-32 w-full rounded-xl" />
        </div>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-3 sm:px-6 py-6 max-w-5xl">
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-2 shrink-0 mt-1">
          ...
          <Button variant="outline" size="sm" className="gap-1.5 border-dashed text-muted-foreground hover:text-foreground" onClick={fillInspectionDemoData}>
            <Wand2 className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Fill Demo Data</span>
          </Button>
          <Button variant="outline" size="sm" className="text-muted-foreground hover:text-foreground hidden sm:flex" onClick={() => {
        ...
        <CollapsibleSection title="Inspection Details" open={openSections.details} onToggle={() => toggleSection('details')}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <Input type="date" value={form.inspection_date} onChange={e => setField('inspection_date', e.target.value)} />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Inspection Type *</label>
              <Select value={form.inspection_type} onValueChange={v => { setField('inspection_type', v); setFieldErrors(prev => ({ ...prev, inspection_type: undefined })); }}>
                <SelectTrigger className={`h-9 text-sm ${fieldErrors.inspection_type ? 'border-destructive' : ''}`}><SelectValue placeholder="Select type..." /></SelectTrigger>
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {sortedChipKeys.map(key => (
            <button key={key} type="button" onClick={() => {
              ...
              if (!vType) return;
              setForm(prev => prev ? { ...prev, violations: [...prev.violations, { ...newViolation(), violationKey: key, correctiveAction: vType.defaultCorrectiveAction, dueDate: calcDueDate(prev.inspection_date, vType) }] } : null);
            }} className="px-3 py-1.5 rounded-full text-xs font-medium border border-border bg-card hover:bg-muted">+ {COMMON_VIOLATION_LABELS[key]}</button>
          ))}
        </div>
        <div className="space-y-3">
          {form.violations.map((v, i) => (
            <ViolationRow key={v.id} violation={v} index={i} inspectionDate={form.inspection_date} onChange={handleViolationChange} onRemove={handleViolationRemove} />
          ))}
        </div>
        <Button type="button" variant="outline" size="sm" className="mt-4 gap-2" onClick={handleAddViolation}><Plus className="w-4 h-4" /> Add Violation</Button>
      </CollapsibleSection>

      <CollapsibleSection title="Observations & Summary" open={openSections.observations} onToggle={() => toggleSection('observations')}>
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Global Observations</label>
            <div className="flex items-center gap-2">
              <Input placeholder="Add a global observation…" value={globalObsInput} onChange={e => setGlobalObsInput(e.target.value)} />
              <Button type="button" variant="outline" onClick={addGlobalObs}>Add</Button>
            </div>
          </div>
          <div className="space-y-2">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Summary / Observations</label>
            <Textarea placeholder="Describe key findings..." value={form.summary} onChange={e => setField('summary', e.target.value)} className="min-h-[120px]" />
          </div>
        </div>
      </CollapsibleSection>

      <CollapsibleSection title="Photos" icon={<Camera className="w-4 h-4" />} open={openSections.photos} onToggle={() => toggleSection('photos')}>
        <PhotoUploadSection complaintId={form.complaintid} inspector={inspectorName} photos={photos} onPhotosChange={setPhotos} />
      </CollapsibleSection>
      ...
      <CollapsibleSection title="Hearing Preparation" open={openSections.hearing} onToggle={() => toggleSection('hearing')}>
        <div className="space-y-4">
          ...
          {form.hearingReferral && <Textarea placeholder="Hearing notes..." value={form.hearingNotes} onChange={e => setField('hearingNotes', e.target.value)} />}
        </div>
      </CollapsibleSection>

      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 pb-10">
        <Button variant="outline" size="lg" className="gap-2 w-full sm:w-auto" onClick={() => handleSave(true)} disabled={saving}>
          {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Save Draft
        </Button>
        <div className="flex gap-2 w-full sm:w-auto">
          <Button variant="outline" size="lg" className="gap-2 flex-1 sm:flex-none" onClick={handlePrint}><Printer className="w-4 h-4" /> Print</Button>
