...
const HEADER_MAP: Record<string, string> = {
  ...
  // 311 Case Number
  "311 case #": "caseNumber311",
  "311 case number": "caseNumber311",
  casenumber311: "caseNumber311",
  ...
  "assigned inspector": "assignedTo",
  inspector: "assignedTo",
  assigned_inspector: "assignedTo",
  ...
  "assigned program": "assignedProgram",
  assigned_program: "assignedProgram",
  "ehb program": "assignedProgram",
  ehb_program: "assignedProgram",
  // Date Assigned
...
function parseCSV(text: string): { rows: ParsedRow[]; strippedCount: number } {
  ...
  const headers = rawHeaders.map(
    (h) => HEADER_MAP[h.toLowerCase().trim()] ?? h.toLowerCase().trim(),
  );
  ...
  const allRows: ParsedRow[] = lines.slice(1).map((line, i) => {
    ...
    return {
      ...
      complaintId: mapped.legacy_complaint_id || undefined,
  ...
  const rows = dataRows.map((r) => {
    ...
    if (seenIds.has(key)) {
      ...
      return {
        ...
        errors: [
          ...r.errors,
          `Duplicate Complaint Number "${r.complaintId}" — first seen on row ${firstRow}`,
        ],
        valid: false,
      };
    }
              ...
              >
                ...
                <td className="px-3 py-2.5">
                  {row.valid ? (
                    ...
                    <span
                      className="bg-destructive/10 text-destructive px-1.5 py-0.5 rounded-full"
...
export default function ImportComplaintsPage() {
  ...
  const bulkImportComplaints = async ({ rows }: { rows: any[] }) => {
    let created = 0;
    const errors: { row: number; reason: string }[] = [];
    for (const row of rows) {
      try {
        await complaintService.create(row);
        created++;
      } catch (err) {
        errors.push({ row: 0, reason: (err as Error).message });
      }
    }
  ...
  const downloadTemplate = () => {
    ...
    a.href = url;
    a.download = "complaint-import-template.csv";
    a.click();
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 py-8 max-w-3xl">
      ...
      {result && (
        <div
          className={`mb-6 rounded-xl p-5 border flex items-start gap-4 ${result.created > 0 ? "bg-primary/5 border-primary/20" : "bg-muted border-border"}`}
        >
        ...
        <div className="space-y-4">
          {showStrippedBanner && (
            <div className="flex items-start justify-between gap-3 bg-muted border border-border rounded-lg px-4 py-3">
              ...
              <button
                onClick={() => setShowStrippedBanner(false)}
        ...
        <div className="mt-8 bg-muted/40 border border-border rounded-xl p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1.5">
            {[
              ...
              { col: "Date Received", req: true },
              { col: "Complaint Details", req: true },
              { col: "Complaint Number", req: false },
              { col: "Location ID", req: false },
              { col: "311 Case Number", req: false },
              { col: "Complaint Type", req: false },
              { col: "Complaint Category", req: false },
              { col: "Assigned To", req: false },
              { col: "Status", req: false },
              { col: "Method Received", req: false },
              { col: "EHB Program", req: false },
            ].map(({ col, req }) => (
              <div key={col} className="flex items-center gap-2">
                <span
                  className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${req ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                >
                  {req ? "REQ" : "OPT"}
                </span>
