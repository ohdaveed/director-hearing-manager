...
const SF_STREETS = [
  'Mission St', 'Valencia St', 'Folsom St', 'Howard St', 'Bryant St',
  'Haight St', 'Divisadero St', 'Fillmore St', 'Turk St', 'Eddy St',
  'Ellis St', 'Post St', 'Sutter St', 'Bush St', 'Geary Blvd',
  'Cesar Chavez St', 'Potrero Ave', 'Guerrero St', 'Church St',
  'Noe St', 'Castro St', 'Clayton St', 'Carl St', 'Stanyan St',
  'Melrose Ave', 'Chenery St', 'Randall St', 'Cortland Ave', 'Precita Ave',
];
const FIRST_NAMES = ['Maria', 'James', 'Linda', 'Robert', 'Patricia', 'Michael', 'Barbara', 'David', 'Jennifer', 'Richard', 'Susan', 'Charles', 'Jessica', 'Thomas', 'Sarah', 'Christopher', 'Karen', 'Daniel', 'Lisa', 'Matthew'];
const LAST_NAMES = ['Garcia', 'Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Miller', 'Davis', 'Wilson', 'Moore', 'Taylor', 'Anderson', 'Thomas', 'Jackson', 'White', 'Harris', 'Martin', 'Thompson', 'Young', 'Lee'];
const CENSUS_TRACTS = ['015.00', '017.00', '019.00', '021.00', '023.00', '025.00', '027.00', '029.00', '031.00', '033.00'];
...
function generateDemoData(inspectorName?: string): Partial<FormState> & { _locationAddress: string } {
  ...
  return {
    ...
    unitNumber: (!isVacantLot && Math.random() > 0.45) ? pick(['1', '2', '3', '4', '5', '1A', '2B', '3C', '101', '202', '305']) : '',
    facilityName: (!isVacantLot && Math.random() > 0.55) ? `${ownerLast} ${pick(['Apartments', 'Hotel', 'Residence'])}` : '',
    ...
    complainantEmail: isAnonymous ? '' : `${firstName.toLowerCase()}.${lastName.toLowerCase()}@gmail.com`,
    complainantAddress: isAnonymous ? '' : `${randInt(100, 999)} ${pick(SF_STREETS)}, San Francisco, CA 94103`,
...
const COMPLAINT_TYPES = ['Animals and Pests', 'Housing Code Violations', 'Vector Control', 'Air Pollutants and Odors', 'Sanitation / Waste', 'Hazardous Conditions', 'Rodents', 'Noise', 'Other'];
...
const CATEGORY_GROUPS: { group: string; items: string[] }[] = [
  ...
  {
    group: 'Sanitation',
    items: ['Garbage / Refuse / Waste / Debris', 'Human / Animal Waste (Sewage)', 'Overgrown Vegetation'],
  },
  {
    group: 'Garbage Area',
...
const HEALTHY_HOUSING_FACILITY_TYPES = new Set(['Apartments', 'Residential Hotel', 'Residential Property']);
...
type RecentLocation = { id: string; address: string; facilityType?: string; ownerName?: string };
...
function saveRecentLocation(loc: RecentLocation) {
  const existing = getRecentLocations().filter(l => l.id !== loc.id);
...
type Action =
  ...
  | { type: 'RESET'; inspectorName?: string };

const today = new Date().toISOString().split('T')[0];

function makeInitialState(inspectorName?: string): FormState {
  return {
    complaintId: '', caseNumber311: '', dateReceived: today,
    locAddress: '', locLocationId: '', locBlockLot: '', locOwnerName: '',
    locOwnerAddress: '', locOwnerPhone: '', locOwnerEmail: '', locFacilityType: '',
    locNumUnits: '', locHealthyHousing: false, locCensusTract: '',
    unitNumber: '', facilityName: '', facilityOwnership: '',
    complaintType: '', complaintSubtype: '', methodReceived: '311', assignedProgram: '',
    dateAssigned: today, description: '', categories: [],
    assignedTo: inspectorName || '',
    complainantAnonymous: false, complainantName: '', complainantPhone: '',
    complainantEmail: '', complainantAddress: '', complainantContactDates: '',
    status: 'New', dateClosed: '',
  };
}

function reducer(state: FormState, action: Action): FormState {
  if (action.type === 'RESET') return makeInitialState(action.inspectorName);
...
function SectionCard({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`bg-card border border-border rounded-xl shadow-sm p-4 sm:p-6 mb-4 sm:mb-6 ${className}`}>
      {children}
    </div>
  );
}

function SectionHeader({ step, done, icon, title, optional }: {
  step: string | React.ReactNode; done: boolean; icon?: React.ReactNode; title: string; optional?: boolean;
}) {
  return (
    <div className="flex items-center gap-2 mb-5">
      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${done ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
        ...
        {steps.map((step, i) => (
          <div key={step.label} className="flex items-center gap-1 flex-1">
            <div className="flex flex-col items-center gap-1 flex-1">
              <div className={`h-1.5 w-full rounded-full transition-all duration-300 ${step.done ? 'bg-primary' : 'bg-muted'}`} />
              <span className={`text-[10px] font-medium ${step.done ? 'text-primary' : 'text-muted-foreground'}`}>{step.label}</span>
            </div>
...
export default function ComplaintEntryPage({ inspectorName, onSuccess }: Props) {
  ...
  const fillDemoData = () => {
    ...
    dispatch({ type: 'RESET', inspectorName });
    // Small delay so reset completes first
    setTimeout(() => {
      Object.entries(demo).forEach(([k, v]) => {
        if (k !== '_locationAddress') {
          dispatch({ type: 'SET_FIELD', field: k as keyof FormState, value: v as FormState[keyof FormState] });
        }
      });
  ...
  const handleReset = () => {
    dispatch({ type: 'RESET', inspectorName });
    setLocationQuery(''); setSelectedLocation(null); setCreatingNewLocation(false);
    setLocationResults([]); setDuplicateComplaints([]); setDuplicateAction(null);
    setLinkedToComplaintId(null); setSubmitted(false); setSubmitAttempted(false);
    ...
    <div className="container mx-auto px-3 sm:px-6 py-6 sm:py-8 max-w-3xl">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Complete all relevant sections from the Environmental Health Branch Complaint Form.</p>
        </div>
        <Button
          variant="outline"
          size="sm"
      ...
      <SectionCard>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">311 Case #</label>
            <Input placeholder="e.g. 101003863368" value={state.caseNumber311} onChange={e => set('caseNumber311', e.target.value)} />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Date Received</label>
            <Input type="date" value={state.dateReceived} onChange={e => set('dateReceived', e.target.value)} />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Method Received</label>
            <Select value={state.methodReceived} onValueChange={v => set('methodReceived', v)}>
          ...
          <div className="space-y-1">
            ...
            <Select value={state.assignedProgram} onValueChange={v => set('assignedProgram', v)}>
          ...
          <div className="space-y-1">
            ...
            <Input type="date" value={state.dateAssigned} onChange={e => set('dateAssigned', e.target.value)} />
          </div>
        </div>
      </SectionCard>
      ...
      <SectionCard>
        <SectionHeader step="2" done={hasLocation} icon={<MapPin className="w-4 h-4 text-primary" />} title="Property Location" />
        {submitAttempted && !hasLocation && (
          <div className="flex items-center gap-1.5 text-xs text-destructive mb-3 -mt-2">
            <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
            A property location is required to save this complaint.
          </div>
        )}
        ...
        {!selectedLocation && !creatingNewLocation && (
          <div className="space-y-3">
            ...
            {recentLocations.length > 0 && !locationQuery && (
              <div>
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2">Recent Locations</p>
                <div className="space-y-1">
            ...
            <div className="relative">
              <Input placeholder="Search by address..." value={locationQuery}
                onChange={e => { setLocationQuery(e.target.value); doLocationSearch(e.target.value); }} className="pr-9" />
              <div className="absolute right-2.5 top-1/2 -translate-y-1/2">
                {isSearchingLocations ? <Loader2 className="w-4 h-4 animate-spin text-muted-foreground" /> : <Search className="w-4 h-4 text-muted-foreground" />}
              </div>
            </div>
            <AnimatePresence>
        ...
        {creatingNewLocation && (
          <div className="space-y-4 mb-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" className="text-xs" onClick={() => setCreatingNewLocation(false)}>Cancel</Button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2 space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Address <span className="text-destructive">*</span></label>
                <Input placeholder="Street address" value={state.locAddress}
                  onChange={e => { set('locAddress', e.target.value); doDuplicateCheckByAddress(e.target.value); }}
                  onBlur={() => setTouched(prev => ({ ...prev, locAddress: true }))}
                  className={touched.locAddress && !state.locAddress ? 'border-destructive' : ''} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Location ID</label>
                <Input placeholder="e.g. 110881" value={state.locLocationId} onChange={e => set('locLocationId', e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Block / Lot</label>
                <Input placeholder="e.g. 1234 / 056" value={state.locBlockLot} onChange={e => set('locBlockLot', e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Facility Type</label>
                <Select value={state.locFacilityType} onValueChange={v => {
              ...
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Census Tract</label>
                <Input placeholder="e.g. 027.00" value={state.locCensusTract} onChange={e => set('locCensusTract', e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1"><Building2 className="w-3 h-3" /> Owner Name</label>
                <Input placeholder="Full name" value={state.locOwnerName} onChange={e => set('locOwnerName', e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Owner Address</label>
                <Input placeholder="Mailing address" value={state.locOwnerAddress} onChange={e => set('locOwnerAddress', e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1"><Phone className="w-3 h-3" /> Owner Phone</label>
                <Input placeholder="(415) 555-1234" value={state.locOwnerPhone} onChange={e => set('locOwnerPhone', e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1"><Mail className="w-3 h-3" /> Owner Email</label>
                <Input type="email" placeholder="owner@email.com" value={state.locOwnerEmail} onChange={e => set('locOwnerEmail', e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide"># Units</label>
                <Input type="number" min="0" placeholder="0" value={state.locNumUnits} onChange={e => {
              ...
              <div className="sm:col-span-2">
                <label className="flex items-center gap-2 cursor-pointer text-sm select-none">
                  <Checkbox checked={state.locHealthyHousing} onCheckedChange={v => set('locHealthyHousing', !!v)} />
        ...
        {hasLocation && (
          <div className="pt-4 border-t border-border mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-1">
              ...
              <Input placeholder="e.g. 3B" value={state.unitNumber} onChange={e => set('unitNumber', e.target.value)} />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Facility Name</label>
              <Input placeholder="Optional" value={state.facilityName} onChange={e => set('facilityName', e.target.value)} />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Facility Ownership</label>
              <Input placeholder="Owner / management" value={state.facilityOwnership} onChange={e => set('facilityOwnership', e.target.value)} />
            </div>
          </div>
        )}
      </SectionCard>
      ...
      <AnimatePresence>
        {(isDuplicateChecking || duplicateComplaints.length > 0) && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mb-4 sm:mb-6">
            {isDuplicateChecking ? (
              <div className="flex items-center gap-2 text-sm text-muted-foreground px-1">
                <Loader2 className="w-4 h-4 animate-spin" /> Checking for existing complaints...
              </div>
              ...
              <div className="bg-warning/10 border border-warning/30 rounded-xl p-4 sm:p-5">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    <p className="font-semibold text-foreground text-sm">Open complaint already on file at this address</p>
                    <p className="text-xs text-muted-foreground mt-0.5">Link this intake to it, or save as a separate record.</p>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <Button size="sm" className="gap-2" onClick={() => setDuplicateAction('link')}><Link2 className="w-4 h-4" /> Link to Existing</Button>
                  <Button size="sm" variant="outline" className="gap-2" onClick={() => setDuplicateAction('standalone')}><FileText className="w-4 h-4" /> Save as Standalone</Button>
                </div>
              </div>
            ...
            ) : duplicateComplaints.length > 0 && duplicateAction === 'standalone' ? (
              <div className="flex items-center justify-between bg-muted border border-border rounded-xl px-4 py-3">
                <div className="flex items-center gap-2">
                  <FileText className="w-4 h-4 text-muted-foreground" />
                  <span className="text-sm text-muted-foreground">Will be saved as a standalone complaint</span>
                </div>
                <Button variant="ghost" size="sm" className="text-xs" onClick={() => setDuplicateAction(null)}>Change</Button>
              </div>
            ) : null}
      ...
      <SectionCard>
        <SectionHeader step="3" done={hasComplainant} icon={<User className="w-4 h-4 text-primary" />} title="Complainant Information" optional />
        <label className="flex items-center gap-2.5 cursor-pointer select-none mb-4 p-3 rounded-lg bg-muted/50 border border-border">
          <Checkbox checked={state.complainantAnonymous} onCheckedChange={v => set('complainantAnonymous', !!v)} />
          <span className="text-sm font-medium">Anonymous Complainant</span>
          {state.complainantAnonymous && <span className="ml-auto text-xs bg-muted px-2 py-0.5 rounded-full text-muted-foreground font-medium">Anonymous ☑</span>}
        </label>
        <AnimatePresence>
          {!state.complainantAnonymous && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2 space-y-1">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1"><User className="w-3 h-3" /> Name</label>
                  <Input placeholder="Full name" value={state.complainantName} onChange={e => set('complainantName', e.target.value)} />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1"><Phone className="w-3 h-3" /> Phone</label>
                  <Input placeholder="(415) 555-5678" value={state.complainantPhone} onChange={e => set('complainantPhone', e.target.value)} />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1"><Mail className="w-3 h-3" /> Email</label>
                  <Input type="email" placeholder="complainant@email.com" value={state.complainantEmail}
                    onChange={e => { set('complainantEmail', e.target.value); if (validateEmail(e.target.value)) clearError('complainantEmail'); }}
                    onBlur={() => blurField('complainantEmail')}
                    className={touched.complainantEmail && formErrors.complainantEmail ? 'border-destructive' : ''} />
                  {touched.complainantEmail && formErrors.complainantEmail && (
                    <p className="text-xs text-destructive mt-1">{formErrors.complainantEmail}</p>
                  )}
                </div>
                <div className="sm:col-span-2 space-y-1">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1"><MapPin className="w-3 h-3" /> Address</label>
                  <Input placeholder="Mailing address" value={state.complainantAddress} onChange={e => set('complainantAddress', e.target.value)} />
                </div>
                <div className="sm:col-span-2 space-y-1">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1"><Calendar className="w-3 h-3" /> Contact Dates</label>
                  <Input placeholder="e.g. 4/2/26, 4/15/26" value={state.complainantContactDates} onChange={e => set('complainantContactDates', e.target.value)} />
                </div>
              </div>
            </motion.div>
          )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <Select value={state.complaintType} onValueChange={v => {
                set('complaintType', v);
                // Smart default: auto-set program
                const prog = COMPLAINT_TYPE_TO_PROGRAM[v];
                if (prog && !state.assignedProgram) set('assignedProgram', prog);
            ...
            <div className="space-y-1">
              ...
              <Input placeholder="e.g. Rodent infestation" value={state.complaintSubtype} onChange={e => {
                const val = e.target.value;
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Inspector Notes</label>
            <Textarea
              placeholder="Internal notes (not part of the complaint description)..."
              className="min-h-[72px] resize-none"
              disabled
            />
            <p className="text-[10px] text-muted-foreground">Inspector notes can be added via the Chronology after saving.</p>
          </div>
          <fieldset>
            <legend className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-3">Category</legend>
            <div className="space-y-3">
              {CATEGORY_GROUPS.map(({ group, items }) => (
                <div key={group}>
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5">{group}</p>
                    ...
                    {items.map(cat => (
                      <label key={cat} className="flex items-center gap-2 cursor-pointer text-sm select-none">
                        <Checkbox
                          ...
                          onCheckedChange={checked => set('categories', checked ? [...state.categories, cat] : state.categories.filter(c => c !== cat))}
                        />
                        {cat}
                      </label>
                    ))}
