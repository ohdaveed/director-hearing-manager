-- ═════════════════════════════════════════════════════════════════════════════
-- Migration 004: Standardize Foreign Key Column Names
-- ═════════════════════════════════════════════════════════════════════════════
-- Aligns with "Approach A: Standard _id suffix"
-- 1. Descriptive text fields -> legacy_*_ref
-- 2. Legacy text IDs -> legacy_*_id
-- 3. UUID/BIGINT FKs -> *_id
-- ═════════════════════════════════════════════════════════════════════════════
...
-- inspections
ALTER TABLE inspections RENAME COLUMN complaint_id_uuid TO complaint_id;
ALTER TABLE inspections RENAME COLUMN location_id_uuid TO location_id;

-- violations (already renamed in live DB? check if inspection_id exists)
