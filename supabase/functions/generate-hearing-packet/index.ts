...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!id) return new Response(JSON.stringify({ error: "packetId is required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const admin = createClient(Deno.env.get("SUPABASE_URL") ?? "", Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "");
    const { data: packetJson, error: packetError } = await admin.rpc("build_hearing_packet_json", { p_hearing_packet_id: id });
    if (packetError) throw packetError;
    const { data: validationResults, error: validationError } = await admin.rpc("validate_hearing_packet", { p_hearing_packet_id: id });
    ...
    const { data: upload, error: uploadError } = await admin.storage.from("hearing-packets").upload(filePath, new Blob([html], { type: "text/html" }), { contentType: "text/html; charset=utf-8", cacheControl: "3600", upsert: false });
    if (uploadError) throw uploadError;
    const { data: signed } = await admin.storage.from("hearing-packets").createSignedUrl(filePath, 3600);

    const { data: fileRecord, error: fileError } = await admin.from("generated_packet_files").insert({
      ...
      metadata: { storageBucket: "hearing-packets", storagePath: upload.path, packetType, validationResults },
    }).select("*").single();
    if (fileError) throw fileError;

    await admin.from("packet_generation_events").insert({ hearing_packet_id: id, complaint_uuid: packetJson?.complaint_uuid ?? caseData.complaint_uuid ?? null, event_type: packetType === "final" ? "final_packet_generated" : "draft_packet_generated", event_status: "success", event_message: `${packetType} hearing packet generated and stored.`, event_data: { fileRecordId: fileRecord.id, storagePath: upload.path } });
    await admin.from("hearing_packets").update({ [packetType === "final" ? "final_file_path" : "generated_file_path"]: signed?.signedUrl ?? upload.path, packet_snapshot_json: packetJson, validation_results_json: validationResults ?? [], generated_at: now.toISOString(), ...(packetType === "final" ? { packet_status: "Complete" } : {}) }).eq("id", id);

    return new Response(JSON.stringify({ ok: true, packetId: id, packetType, file: fileRecord, signedUrl: signed?.signedUrl ?? null, validationResults: validationResults ?? [] }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
    console.error(error);
